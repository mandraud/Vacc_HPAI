...
initLots_Gras<-function(nNonV3=1000,nV3=0,id1=0,Sale,minV1=10,maxV1=21,minDelayV2=18,maxDelayV2=23,Age_V3 = 56){
  values <- minV1:maxV1
  ...
  lots=data.frame('id' = id1+(1:n),
                ...
                "EliV3"=c(rep(TRUE,nV3),rep(FALSE,nNonV3))# "AgeV2"=sample(30:40,n,replace = T), 
                )
  tmp=runif(n)
  ...
  lots$AgeV3 =  NA
  if (nV3>0){
    lots$AgeV3[1:nV3]=sample((Age_V3-5):Age_V3, 
                           size = nV3, replace = TRUE)
  }
  lots$AgeCrit= lots$AgeV2 + 7 + 42
  lots$statut=factor(lots$statut,levels=c("NV","NP","PP","FP","FPV3","FP2V3"))
lots
  }


Vaccination_Modele_Gras<- function(nweeks=26,nNonV3=1,nV3=0,Sale,MEP){
  
  
  lots=initLots_Gras(nNonV3=nNonV3,nV3=nV3,Sale=Sale)
...
for (t in 2:(nweeks*7+1)){
  ...
  if (any((lots$EliV3 & lots$Age==lots$AgeV3)->index)) lots$statut[index]="FPV3"
  if (any((lots$statut=="FPV3" & lots$Age==lots$AgeCrit)->index)) lots$statut[index]="FP2V3"
  if (any(lots$Age==lots$AgeCull)) {
    
    lots=lots[-which(lots$Age==lots$AgeCull),]
  }
  if (any(MEP[,1]==t)){
  intro=initLots_Gras(MEP[which(MEP[,1]==t),2],MEP[which(MEP[,1]==t),3],id1=max(lots$id),Sale=Sale)}
  intro$Age=1
...
levels(res2$statut)=c("Animals too young to be vaccinated","end of protection",
                      "Partial protection","Full protection","Full protection",
                      "Full protection thanks to the third injection")
# levels(res2$statut)[4:5]="Full protection"
Plot1<-ggplot(res2, aes(x=week,fill=statut))+geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent)+ 
  scale_fill_manual(values=c("orange3", "seashell4", "seagreen2", "seagreen4", "chartreuse3"))+
  xlab("Week")+ylab('Percentage of batches')+ 
  theme(legend.position = "bottom",legend.box = "horizontal") +
  guides( fill = guide_legend(title = NULL,ncol = 3, byrow=TRUE))
